...
int ANGULAR_SPEED=1;					//angular speed: 142.86 degree/sec, time to complete a full rotation: 2.52 sec
int ZOOMING=0.7;						//rate at which rectangle expands: 166.67 pixles/sec
...
GLfloat cactposmsg[]={1200.0,2000.0,2800.0,3750.0,5020.0,6210.0,6980.0,7790.0,8630.0,9500.0,10300.0,11180.0,12070.0,12880.0,13750.0,14580.0,15430.0,16200.0,
...
//Text to display on cover screen

char textc[]={"CGV Mini Project"};
char textd[]={"Dino Run"};
char texte[]={"<<---------------------- INSTRUCTIONS ---------------------->>"};
char textf[]={"Press 'W' or Spacebar Key to make Dino Jump"};
char textg[]={"Press 'R' to Restart the game"};
char texth[]={"Press 'S' to start the game"};
...
void drawScnFreez()					//displaying the scoreand GAME OVER
{
	...
	glClearColor(0.0,0.0,0.0,0.0);
	glColor3f(1.0,1.0,1.0);
	drawText(750,350,"GAME OVER");			//display GAME OVER message
...
void killSrcnSquareRotation(int value)			//angle updater for kill screen square
{
	...
	glutPostRedisplay(); 				// Inform GLUT that the display has changed;
	glutTimerFunc(ANGULAR_SPEED,killSrcnSquareRotation,0);
}
void killSrcnSquareScaling(int value)			//scaling the kill screen square
{
	...
	glutPostRedisplay(); 				// Inform GLUT that the display has changed;
	glutTimerFunc(ZOOMING,killSrcnSquareScaling,0);
}
...
void display()
{
	...
	if(gameStart==1)				//if the game has started
	{
    	...
    	{
        	drawCoverPage();
        	glutPostRedisplay();			//change has been made to the display so re display it
       	}
...
void reset()						//initialize or reset the value of constants to start a new game
{
	int i;
...
void menusystem()					//main menu description function
{
	...
	glutAddSubMenu("Dino color",texchangeid);
	glutAddSubMenu("Terrain",terchangeid);
	glutAddSubMenu("Traveling speed",speedchangeid);
...
void myinit()						//initialization function
{
...
int main(int argc,char** argv)				//MAIN FUNCION
{
	...
	glutCreateWindow("Dino Run");		//window title name
